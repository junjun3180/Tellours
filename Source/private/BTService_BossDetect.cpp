...
void UBTService_BossDetect::TickNode(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
	...
	if (bResult)
	{
		for (auto const& OverlapResult : OverlapResults) {
			...
			{	
				OwnerComp.GetBlackboardComponent()->SetValueAsObject(ATOBossController::TargetKey, TOCharacter);
	...
	OwnerComp.GetBlackboardComponent()->SetValueAsObject(ATOBossController::TargetKey, nullptr);
