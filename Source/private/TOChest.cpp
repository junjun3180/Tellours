...
ATOChest::ATOChest()
{
	...
	Chest->SetMassOverrideInKg("", 10000);
...
void ATOChest::ChestOpen(ATOCharacter* Character)
{
	...
	if (!RequiredKey || Character->CharacterStat->GetInventoryItemCount()[0])
	{
		...
		// 아이템 드랍
		ATOItem* DropedItem = (ATOItem*)GetWorld()->SpawnActor<ATOItem>(GetActorLocation() + FVector::UpVector * 50, FRotator::ZeroRotator);
		DropedItem->ItemDrop();

		// 상자 삭제, 재생성
		GetWorld()->GetTimerManager().SetTimer(ChestDeadTimeHandle, FTimerDelegate::CreateLambda([&]() {Destroy(); }), 10, false);
